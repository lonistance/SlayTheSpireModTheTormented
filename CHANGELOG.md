...
- **Verification**: full javac compile EXIT=0;  validate_localization.ps1 PASS in all 24 languages.
**#33 HeroLongsword crash fix: ConcurrentModificationException after picking the relic**
- **Symptom**: picking HeroLongsword crashed the game with `java.util.ConcurrentModificationException` at `OverlayMenu.update(OverlayMenu.java:69)` (`ArrayList$Itr.next`) right after the relic landing animation finished.
- **Root cause (bytecode-verified)**: `OverlayMenu.update()` iterates `AbstractDungeon.player.relics` every frame with a for-each calling `r.update()` (source line 69; bytecode PC 82-119). The landing of a newly obtained relic happens inside that loop: vanilla `AbstractRelic.update()` detects the `isAnimating` flight reaching `targetX/targetY`, sets `isDone = true` and calls `onEquip()` (bytecode PC 376-437). Our `HeroLongsword.onEquip()` called `AbstractDungeon.player.loseRelic(CursedBrokenBlade.ID)`, which structurally removes that relic from the very list being iterated - the next `Iterator.next()` then throws CME. (Vanilla `loseRelic` itself is an iterator-safe pattern: it scans first and only removes after its own loop; the crash is provoked by the outer OverlayMenu iterator.)
- **Fix**: `HeroLongsword.onEquip()` no longer removes the relic synchronously. Starter Strike/Defend upgrades still run immediately inside `onEquip`; the `loseRelic(CursedBrokenBlade.ID)` call is deferred into a one-shot `AbstractGameEffect` (0.05 s) added to `AbstractDungeon.effectList` - the effect ticks next frame when no `player.relics` iterator is alive, and CursedBrokenBlade is removed exactly as designed.
- **Verification**: full javac compile EXIT=0 (151 sources). No localization changes (no validate run needed). Awaiting in-game confirmation: take HeroLongsword from the boss relic screen - the broken blade is removed after the landing animation and the game no longer crashes.
**#34 CharacterStrings overhaul: real character description, SpireHeart attack line, verbatim Vampires greeting (all 24 languages)**
- **Content**: rewrote the three CharacterStrings TEXT fields for The Tormented in all 24 languages.
  - **TEXT[0] (character select flavor)**: replaced the old one-liner with a 4-sentence description built from the character's background story (Steam workshop lore): the doomed warrior prophesied as a child of calamity whose eternal curse makes war and death follow wherever he walks, his bargain and climb to end the catastrophe that befell the world, the Sin/Debt/Bleed mechanics using each language's in-game power names, closing with "this time he fights for redemption, not glory".
  - **TEXT[1] (SpireHeart weapon attack, `getSpireHeartText`)**: new atonement line — slaying the Heart completes the bargain and lifts his curse, "that is his only path to redemption", then "You raise the sword in your hand."
  - **TEXT[2] (Vampires greeting, `getVampireText`)**: now a verbatim copy of the vanilla `Vampires` event's `DESCRIPTIONS[0]` per language (extracted from `desktop-1.0.jar`), so the event addresses the (male) character correctly — e.g. zhs "加入我们，兄弟"; previously it was leftover English in the zhs file. Byte-verified identical to the vanilla string for all 24 languages.
- **Technical**: texts were authored in a TSV map and applied by a one-off PS script; JSON files rewritten in the repo's existing format (CRLF, 2-space indent, UTF-8 no BOM). TEXT[2] was copied programmatically from the jar extraction, guaranteeing verbatim fidelity.
- **Follow-up fix (crash)**: the initial application script corrupted the JSON top-level key of all 24 files to `"$"` (PowerShell string interpolation ate `${modID}`) and blanked NAMES; the game then crashed on startup with NPE at `Tormented.getNames(Tormented.java:45)` (`getCharacterString("thetormented:TheTormented")` returned null). All 24 files were rebuilt: key and NAMES restored verbatim from HEAD, new TEXT preserved (recovered from the corrupted files). Re-verified: no BOM, key/NAMES/TEXT intact, and `validate_localization.ps1` PASS in all 24 languages again.
- **Verification**: `validate_localization.ps1` PASS for all 24 languages (eng 627 / zhs 3 legacy English-word warnings are the pre-existing baseline, not introduced by this batch).

**#35 Custom Heart-ending cutscene framework (art pending): The Tormented no longer borrows Ironclad's ending artwork**
- **Problem**: after beating the Heart (SpireHeart), the vanilla game enters `TrueVictoryRoom`, whose constructor runs `new Cutscene(player.chosenClass)`; the `Cutscene` constructor switches on `PlayerClass` (1-4 = the four vanilla characters) and its `default:` branch falls back to Ironclad's ending art (`images/scenes/redBg.jpg` + `ironclad1-3.png`) — so a mod character always saw Ironclad's victory cutscene. Bytecode-verified: `TrueVictoryRoom.<init>` (PC 12-25) creates the cutscene; `Cutscene` constructor tableswitch 1/2/3/4 -> ironclad/silent/defect/watcher panels, `default` -> ironclad; `CutscenePanel` loads its texture via `ImageMaster.loadImage`, which returns null (log-only) when the file is missing.
- **Fix**: new `thetormented/patches/VictoryCutscenePatch.java` — a postfix on `Cutscene.<init>` (paramtypez `{AbstractPlayer.PlayerClass.class}`). When `AbstractDungeon.player.chosenClass == Tormented.Meta.TORMENTED`, it loads the mod's own background + 3 panels and replaces the private fields `bgImg` / `panels` via `ReflectionHacks.setPrivate`; if any texture fails to load, it aborts and the vanilla (Ironclad) cutscene remains, so missing art can never crash the game. Art file convention (drop into `src/main/resources/thetormented/images/character/`): `endingBg.jpg` (fullscreen background, 1920x1080) and `ending1.png` / `ending2.png` / `ending3.png` (panel art; non-16:10 rendering area is WIDTH x (HEIGHT + 110 x scale), per `Cutscene.renderImg`).
- **Art delivered**: `endingBg.jpg` + `ending1/2/3.png`, all 1920x1200 (16:10, fullscreen-safe on 16:10 : uses the vanilla fullscreen draw branch, no cropping).
- **Preview verification (standalone tool, not part of the repo)**: a libGDX/LWJGL2 desktop app ran against `desktop-1.0.jar` (resources resolved exactly like in-game: LWJGL2 internal-files = classpath, mod paths resolved from the resource dir) with the bytecode-exact reproduction of `Cutscene.update`/`updateSceneChange`/`updateFadeIn`/`updateFadeOut`/`updateIfDone` and `CutscenePanel.update`/`activate`/`fadeOut`/`render` (including the vanilla `isDone` loop that re-invokes `fadeOut()` every frame as the original does). Result: all 4 `ImageMaster.loadImage` calls OK (1920x1200), full timeline (blackout ~1s -> P1 5s -> P2 -> P3 -> all fade out -> bg fade -> victory screen) ran to completion with zero exceptions; the preview-only `ImageMaster.WHITE_SQUARE_IMG` null (artifact of the bare environment, never null in-game) was substituted with a self-built 1x1 white texture.
- **Verification**: full javac compile EXIT=0 (180 classes). No localization changes. Jar rebuilt (`target/thetormented-cutscene.jar`, 909 entries / 180 classes), deployed to `mods/thetormented.jar` (previous version backed up as `thetormented.jar.bak-1.0.2-pre-cutscene`), `copy/thetormented.jar` synced. Pending in-game confirmation by the user after beating the Heart with The Tormented.

**#36 Beta art preview: disabling beta art now restores the normal portrait in the open popup (#19 follow-up)**
- **Symptom**: starting the game with global beta art enabled, then disabling beta art (global setting or the per-card toggle inside the preview), left the already-open `SingleCardViewPopup` showing the stale beta portrait; the Card Library grid switched back correctly. The reverse direction (off -> on) worked.
- **Root cause**: #19's `EnforceBetaPortrait` `update()` postfix handled only the ON direction — when `wantBetaArt(card)` was false it merely nulled its `lastBetaPortrait` marker and returned, so `portraitImg` kept holding the old beta texture and nothing re-ran the vanilla/BaseMod portrait loading for the still-open popup.
- **Fix**: on the beta-off frame the patch now actively restores the normal portrait: dispose the owned beta texture, clear `portraitImg`, re-run vanilla `loadPortraitImg()` via `ReflectionHacks.privateMethod`, and — when vanilla leaves it null (mod cards have no `1024Portraits` file) — fill the official `_p` large portrait exactly like BaseMod's `OpenFix$OpenTextureFix` does at open time, via the public API `CustomCard.getPortraitImage(card)` (no path guessing; that BaseMod fix only hooks `open()`, so it never re-runs mid-session on its own).

**#37 Beta art preview rework (#36 follow-up): state-machine driven, strict texture ownership**
- **Problem with #36**: restoration was gated on `cur == lastBetaPortrait` — an identity check that is not a reliable "is the current portrait ours" test. `lastBetaPortrait` is a static field (reset every JVM start) while the slot can be filled by vanilla, BaseMod's `OpenFix$OpenTextureFix`, or even BaseMod's own beta-portrait selection inside `CustomCard.getPortraitImage` (`PLAYTESTER_ART_MODE` / `betaCardPref` -> `_b_p`), so identity-based gating could skip a needed restore; conversely `applyBetaPortrait` disposed whatever texture it found in the slot, which can be a **shared cached texture** (`BaseCard.getPortraitImage()` returns `TextureLoader`-cached textures when global playtester mode is on) — disposing it would poison the cache for the card grid and other viewers.
- **Fix**: logic split into two independent concerns. (1) *State machine*: `trackedCard` / `trackedBetaState` / `hasTrackedState` detect card switches and beta on/off transitions (including first sight after game restart); transitions trigger exactly one `applyBetaPortrait` (beta ON) or `restoreNormalPortrait` (beta OFF). With beta ON and no transition, the portrait is re-applied only when the slot no longer holds our texture (upgrade-preview overwrite fight); with beta OFF and no transition, the hook does nothing. (2) *Ownership*: only textures identical to `lastBetaPortrait` are ever disposed by this patch — everywhere (apply / drop / restore); foreign textures are only overwritten by reference, matching vanilla's own leak-on-reload semantics. `restoreNormalPortrait` no longer inspects ownership to decide whether to act: it disposes our texture if present, unconditionally clears the slot, re-runs vanilla `loadPortraitImg()`, and falls back to the public `CustomCard.getPortraitImage(card)` mirror of BaseMod's `OpenFix$OpenTextureFix` when vanilla leaves it null. `close()` resets the state machine without disposing (vanilla close already disposes `portraitImg`, bytecode-verified).
- **Verification**: full javac compile passes (EXIT=0). No localization changes.

**#38 ExecutionForm <-> Bloodbath effect swap (cards, powers, icons, all 24 languages)**
- **Change**: the two rare Power cards traded mechanics. ExecutionForm (cost 3) is now the bleed engine: the first 1 (2 after upgrade) Attack played each turn applies Bleed equal to its unblocked damage. Bloodbath (cost 2) is now Ethereal and carries the threshold buff: you deal 25% more damage to enemies below 50% (75% upgraded) of their Max HP; the old cost-reduction upgrade (2 -> 1) went away together with the old mechanic.
- **Powers renamed to follow their card**: the threshold logic (formerly JudgmentFormPower) now lives in BloodbathPower; the bleed logic (the old BloodbathPower) now lives in ExecutionFormPower. POWER_IDs derive from the class names: thetormented:ExecutionFormPower / thetormented:BloodbathPower.
- **Icons swapped accordingly**: JudgmentFormPower.png became BloodbathPower.png and BloodbathPower.png became ExecutionFormPower.png (normal + large/); no JudgmentFormPower files remain.
- **Localization (all 24 languages)**: PowerStrings - key ${modID}:JudgmentFormPower renamed to ${modID}:ExecutionFormPower, the two entries' DESCRIPTIONS swapped, NAMEs kept per language. CardStrings - the two cards' DESCRIPTION/UPGRADE_DESCRIPTION swapped; ExecutionForm's UPGRADE_DESCRIPTION was newly written per language ("first 2 attack cards ..."). Also fixed a pre-existing zhs/zht defect where the bleed power description concatenated the amount mid-sentence ("...第一张攻击牌1对敌人..."); both segments now leave room for the amount ("前 1/2 张攻击牌").
- **Verification**: validate_localization.ps1 PASS for all 24 languages; full javac compile EXIT=0 (179 classes); jar rebuilt (target/thetormented-swap2.jar, 908 entries) and deployed to mods/thetormented.jar (previous version backed up as thetormented.jar.bak-1.0.2-pre-swap).

**#40 BaseCard beta-art mode sync: instance-registry broadcast fixes CardLibrary + combat deck stale art**
- **Problem**: when starting with global Beta Art ON then disabling it (global setting or per-card toggle), Rebel and other Tormented cards in the Card Library grid and in the combat draw/discard piles retained stale beta art. The card popup (SingleCardViewPopup) restored correctly. Root cause: `BaseCard.update()` used a single `static lastPlaytesterMode` flag — once any card instance (typically a combat hand copy) consumed the flag, all other instances (library singletons, draw/discard pile cards) never detected the mode change.
- **Fix**: replaced the one-shot static flag with a version-broadcast architecture. A `WeakHashMap`-based `ALL_INSTANCES` registry tracks all live `BaseCard` instances (auto-cleaned on GC). A `static globalModeVersion` counter increments on mode flip; each instance holds its own `syncedModeVersion`. When any instance's `update()` detects a mismatch, it calls `notifyGlobalModeChanged()` which iterates the registry and forces `syncToCurrentMode()` (loadCardImage + refreshJokePortrait) on every live instance — including library singletons and draw/discard pile cards that don't get regular `update()` calls.
- **Verification**: full javac compile EXIT=0 (180 classes). No localization changes (validate not needed). Awaiting in-game confirmation: enable beta art → disable → Card Library grid + combat deck/discard all revert to normal art; Rebel's `jokePortrait` region is updated; SingleCardViewPopup still works correctly.

**#39 v1.0.3 release**
- **Changes in this release** (cumulative #33-#38): HeroLongsword ConcurrentModificationException crash fix (#33); CharacterStrings overhaul - real character description, SpireHeart attack line, verbatim Vampires greeting across all 24 languages (#34); custom Heart-ending cutscene framework with dedicated artwork (#35); Beta-art preview fixes - per-card toggle restore (#36) and the state-machine rework with strict texture ownership (#37); ExecutionForm <-> Bloodbath effect swap with power rename, icon swap and full localization sync (#38).
- **Technical**: pom.xml / ModTheSpire.json version bumped 1.0.2 -> 1.0.3.
- **Verification**: full javac compile EXIT=0 (179 classes); validate_localization.ps1 PASS for all 24 languages; jar deployed to mods/thetormented.jar and to the Steam Workshop content directory.
