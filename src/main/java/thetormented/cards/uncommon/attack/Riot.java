...
public class Riot extends BaseCard {
    public static final String ID = makeID(Crush.class.getSimpleName()); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        int total_damage = debtPower == null ? damage : damage + debtPower.amount * magicNumber;
        for (int i = 0; i < energyOnUse; i++)
            addToBot(new DamageAction(m, new DamageInfo(p, total_damage, DamageInfo.DamageType.NORMAL), AbstractGameAction.AttackEffect.SLASH_VERTICAL));
    }
