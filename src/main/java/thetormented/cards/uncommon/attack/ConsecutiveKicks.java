...
public class ConsecutiveKicks extends BaseCard {
    public static final String ID = makeID(ConsecutiveKicks.class.getSimpleName()); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    private static final int UPG_FREQUENCY = 1;

    public ConsecutiveKicks() {
        super(ID, info); //Pass the required information to the BaseCard constructor.
        setDamage(DAMAGE); //Sets the card's damage and how much it changes when upgraded.
        setMagic(FREQUENCY, UPG_FREQUENCY);
    }
